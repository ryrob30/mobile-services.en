...
* **trackingIdentifier**
  ...
  >[!TIP]
  >
  >Tip:  If your app upgrades from the Experience Cloud 3.x to 4.x SDK, the previous visitor ID (custom or automatically generated) is retrieved and stored as the custom user identifier (see the `getUserIdentifier` row below). This preserves visitor data between upgrades of the SDK. For new installations on the 4.x SDK, the user identifier is `null` and tracking identifier is used.
