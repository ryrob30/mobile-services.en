...
|App ID|a.AppID|c_a_AppID|Stores the application name and version in the `[AppName] [BundleVersion]` format. An example of this format is `myapp 1.1`.|
...
|--- |--- |--- |--- |
|Location (down to 10 km)|`a.loc.lat.a` and `a.loc.lon.a`|`c_a_loc_lat_a` and `c_a_loc_lon_a`|Populated by  trackLocation methods.|
|Location (down to 100 m)|`a.loc.lat.b` and  `a.loc.lon.b`|`c_a_loc_lat_b` and `c_a_loc_lon_b`|Populated by  trackLocation methods.|
|Location (down to 1 m)|`a.loc.lat.c` and  `a.loc.lon.c`|`c_a_loc_lat_c` and `c_a_loc_lon_c`|Populated by  trackLocation methods.|
